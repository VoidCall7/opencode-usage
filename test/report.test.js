...
const {
  ...
  renderText, renderHtml, parseArgs,
...
test("parseArgs 解析全部参数（含 --db）", () => {
  const o = parseArgs(["--open", "--data", "a.jsonl", "--out", "b.html", "--prices", "c.json", "--db", "d.db"]);
  assert.deepEqual(o, { open: true, data: "a.jsonl", out: "b.html", prices: "c.json", db: "d.db" });
});
...
test("aggregate：override 无视自带 cost 重新计算", () => {
  const prices = { "acme/example-model": { input: 1, output: 2, cacheRead: 0, cacheWrite: 0, override: true } };
...
test("renderHtml 内嵌 echarts 与数据、无未替换占位符", () => {
  ...
  assert.ok(html.includes("example-model"));
